...
const Header = () => {
    ...
    return (
        <nav className="navbar navbar-expand-md position-sticky top-0 start-0 end-100">
            <div className="container-fluid d-flex justify-content-between align-items-center">
                ...
                <div id="navbarNav" className={`navbar__menutab collapse navbar-collapse ${screen >= 1024 ? '' : 'position-absolute d-flex flex-column'} ${screen >= 1024 ? '' : clicked ? '' : 'invisible'}`} ref={reference}>
                    <img className={`navbar__closeicon ${screen >= 1024 ? 'invisible' : ''}`} src={CloseIcon} alt="Press to hide the website pages" onClick={() => handleClick()} />
